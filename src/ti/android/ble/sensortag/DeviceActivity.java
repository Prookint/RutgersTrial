...
public class DeviceActivity extends ViewPagerActivity {
  ...
  private static final int GATT_TIMEOUT = 150; // milliseconds
  ...
  private boolean mOriCalibrateRequest = true;
...
//	  		 ACC PERIOD SETTING
			if (confUuid.equals(SensorTag.UUID_ACC_CONF) && enable) {
				changePeriod((byte)1,1);
			}// MAG PERIOD SETTING
			else if (confUuid.equals(SensorTag.UUID_MAG_CONF) && enable) {
				//changePeriod((byte)100,2);
			}// GYR PERIOD SETTING
			else if (confUuid.equals(SensorTag.UUID_GYR_CONF) && enable) {
				//changePeriod((byte)25,3);
			}
//			else if (confUuid.equals(SensorTag.UUID_ORI_CONF) && enable) {
//				calibrateOrientation();
//			}
			
  		BluetoothGattService serv = mBtGatt.getService(servUuid);
  		BluetoothGattCharacteristic charac = serv.getCharacteristic(confUuid);
//  		BluetoothGattCharacteristic datacharac = serv.getCharacteristic(dataUuid);
...
//		mBtLeService.setCharacteristicNotification(datacharac,enable);
//		mBtLeService.waitIdle(GATT_TIMEOUT);
  ...
  private void changePeriod(byte period, int type) {
		 ...
		 case 1 : //accelerometer
			 servUuid = Sensor.ACCELEROMETER.getService();
			 periUuid = SensorTag.UUID_ACC_PERI;
			 break;
		 case 2 : //magnetometer
			 servUuid = Sensor.MAGNETOMETER.getService();
			 periUuid = SensorTag.UUID_MAG_PERI;
			 break;
		 case 3 : //gyro
			 servUuid = Sensor.GYROSCOPE.getService();
			 periUuid = SensorTag.UUID_GYR_PERI;
			 break;
		 }
		 ...
		 {
			 BluetoothGattService serv = mBtGatt.getService(servUuid);
			 BluetoothGattCharacteristic config = serv.getCharacteristic(periUuid);
			 // Write the calibration code to the configuration registers
			 mBtLeService.writeCharacteristic(config,period);
			 mBtLeService.waitIdle(GATT_TIMEOUT);
		 }
...
private void enableNotifications(boolean enable) {
  	for (Sensor sensor : mEnabledSensors) {
  		...
  		mBtLeService.setCharacteristicNotification(charac,enable);
			mBtLeService.waitIdle(GATT_TIMEOUT*4);
  	}
  }
 ...
 private void calibrateBarometer() {
	...
	public void calibrateOrientation() {
		 Log.i(TAG, "calibrate orientation");
		 
		 UUID servUuid = Sensor.ORIENTATION.getService();
		 UUID configUuid = Sensor.ORIENTATION.getConfig();
		 BluetoothGattService serv = mBtGatt.getService(servUuid);
		 BluetoothGattCharacteristic config = serv.getCharacteristic(configUuid);
