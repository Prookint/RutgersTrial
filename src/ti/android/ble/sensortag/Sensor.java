...
public enum Sensor {
  ...
  ACCELEROMETER(UUID_ACC_SERV, UUID_ACC_DATA, UUID_ACC_CONF) {
  	...
  	public Point3D convert(final byte[] value) {
  		...
  		short x = ((short) ((value[0]<<8 |(value[1] & 0x03FF))/4)); // 0x03ff and 4 for 14 bit. for 12bits they must be 0x00ff and 16
  		short y = ((short) ((value[2]<<8 |(value[3] & 0x03FF))/4));
  		short z = ((short) ((value[4]<<8 |(value[5] & 0x03FF))/-4));
  		Log.i("cagdas",Byte.toString(value[2]) + " "+ Byte.toString(value[3]) + " " + Integer.toHexString(x));
  		return new Point3D(x / 1024.0, y / 1024.0, z / 1024.0); //for 2g-12 bit and 8g-14bit constant is 1024. Refer to table1 of KXTJ9 datasheet for he others 
  	}
  },
  ...
  ORIENTATION(UUID_ORI_SERV, UUID_ORI_DATA, UUID_ORI_CONF) {
	  	@Override
	  	public Point3D convert(final byte[] value) {
	        ...
	        float x = shortSignedAtOffset(value, 2) * (500f / 65536f);
	        float z = shortSignedAtOffset(value, 4) * (500f / 65536f);
	  		Log.i("cagdas",Float.toHexString(x) + " "+ Float.toHexString(y) + " " + Float.toHexString(z));
	  		return new Point3D(x, y , z);
	  	}
	  },
  SONAR(UUID_SON_SERV, UUID_SON_DATA, UUID_SON_CONF) {

	  	public Point3D convert(final byte[] value) {

	  		Integer x = shortUnsignedAtOffset(value, 0);
	  		Log.i("cagdas",Byte.toString(value[0]) + " " +Byte.toString(value[1]) + " " + x.toString());
	  		return new Point3D((float)x,0,0);
	  	}
	  };
  ...
  public static final Sensor[] SENSOR_LIST = {IR_TEMPERATURE, ACCELEROMETER, MAGNETOMETER, GYROSCOPE,ORIENTATION,SONAR, HUMIDITY, BAROMETER, SIMPLE_KEYS};
}
